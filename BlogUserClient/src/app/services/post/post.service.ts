import {HttpClient, HttpHeaders} from '@angular/common/http';
import {Inject, Injectable} from '@angular/core';
import {Observable, BehaviorSubject} from 'rxjs';
import {PostModel} from '../../models/PostModel';
import {IResponse} from '../../responses/IResponse';

@Injectable({
    providedIn: 'root'
})

export class PostService {

    private prefix = "posts/";

    constructor(private _client: HttpClient,
                @Inject('HOST_URL') private _host: string) {
    }


    getPosts(): Observable<IResponse<PostModel[]>> {
        return this._client.get<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-posts"
        );
    }

    getAuthorPosts(accountId: number): Observable<IResponse<PostModel[]>> {

        console.log("getAuthorPosts " + accountId);
        let headers = new HttpHeaders().set('content-type', 'application/json');
        return this._client.post<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-author-posts", accountId,
            {headers: headers}
        );
    }

    getPostsByCategoryId(categoryId: number): Observable<IResponse<PostModel[]>> {
        console.log("getPostsByCategoryId(categoryId: number) " + categoryId);
        let headers = new HttpHeaders().set('content-type', 'application/json');
        return this._client.post<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-posts-by-category", categoryId,
            {headers: headers}
        );
    }


    getPost(postId: number): Observable<IResponse<PostModel>> {
        let headers = new HttpHeaders().set('content-type', 'application/json');
        return this._client.post<IResponse<PostModel>>(
            this._host + this.prefix + "get-post", postId,
            {headers: headers}
        );
    }

    getPublishedPostsByAccount(): Observable<IResponse<PostModel[]>> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
        return this._client.get<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-published-posts-by-account", {headers: headers});
    }

    getDraftPostsByAccount(): Observable<IResponse<PostModel[]>> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
        return this._client.get<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-draft-posts-by-account", {headers: headers});
    }

    getPendingPostsByAccount(): Observable<IResponse<PostModel[]>> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
        return this._client.get<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-pending-posts-by-account", {headers: headers});
    }

    getRejectedPostsByAccount(): Observable<IResponse<PostModel[]>> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
        return this._client.get<IResponse<PostModel[]>>(
            this._host + this.prefix + "get-rejected-posts-by-account", {headers: headers});
    }


    addNewPost(title: string, anons: string, fullText: string, categoryId: number): Observable<IResponse> {
        let body = {
            title: title,
            anons: anons,
            fullText: fullText,
            categoryId: categoryId
        };

        console.dir(body);

        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix +"add-post",
            body, {headers: headers}
        );
    }

    addNewPostAndSendToModeration(title: string, anons: string, fullText: string, categoryId: number): Observable<IResponse> {
        let body = {
            title: title,
            anons: anons,
            fullText: fullText,
            categoryId: categoryId

        };

        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix + "add-post-and-send-to-moderation",
            body, {headers: headers}
        );
    }

    updatePost(postId: number, title: string, anons: string, fullText: string, categoryId: number): Observable<IResponse<Object>> {
        let body = {
            postId: postId,
            title: title,
            anons: anons,
            fullText: fullText,
            categoryId: categoryId

        };

        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix + "update-post",
            body, {headers: headers});
    }

    removePost(postId: number): Observable<IResponse> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix + "delete-post",
            postId, {headers: headers});
    }

    sendPostToDraft(postId: number): Observable<IResponse> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix + "send-post-to-draft",
            postId, {headers: headers});
    }

    sendPostModeration(postId: number): Observable<IResponse> {
        let token = localStorage.getItem("token");
        let headers = new HttpHeaders().set("Authorization", "Bearer " + token)
            .set("content-type", "application/json");

        return this._client.post<IResponse>(this._host + this.prefix + "send-post-moderation",
            postId, {headers: headers});
    }
}
